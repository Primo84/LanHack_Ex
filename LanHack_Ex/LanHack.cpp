	...
	switch (code)
	{
		...
		case  WM_COMMAND:
		{
							...
							if (GetAdapterParams(&Adapters[i]) == 0)
							{
								...
								if (OPM != 0)
								{
									...
									if (ret == 0)
									{
										...
										MessageBox(WindowHandle, L"Tryb pracy zosta³ zmieniony pomyœlnie", L"Wiadomoœæ", MB_OK);
									}
									else if(ret==4)
										MessageBox(WindowHandle, L"Przed zmian¹ tego trybu pracy nale¿y roz³¹czyæ siê z sieci¹ WiFi", L"Wiadomoœæ", MB_OK);
									else
										MessageBox(WindowHandle, L"Nie uda³o siê zmieniæ trybu pracy karty sieciowej", L"Wiadomoœæ", MB_OK);


								}
							}
							else
								MessageBox(WindowHandle, L"Nie mo¿na pobraæ informacji o karcie sieciowej", L"Wiadomosc", MB_OK);
						}
						else
			...
			else if (wp == (WPARAM)SaveProtTable->MainWND)
			{
				...
				if (i == 2)
					MessageBox(WindowHandle, L"Tabela pakietów jest pusta", L"Wiadomoœæ", MB_OK);
